#########################
# This code sets up and runs the Pulliam hypothetical migration network metrics
# model data can be found in metric_inputs_adults.xlsx
...
seasons <- 1 # Number of seasons or steps in one annual cycle. 
...
NODENAMES <- c("Node 1", "Node 2")
# Used to name row values in CR outputs, should be ordered to match node order in the .xlsx file

NETNAME <- c("adults") # Give a distinct name for each class as used in input files
...
delta <- -1 #Perturbation amount used in G-metric calculation. delta = -1 means complete node removal
...
node_color <- c("orange", "blue") # MEXICO SOUTH CENTRAL NORTH
NAME <- "Pulliam Metapopulation" # Name for the plots
