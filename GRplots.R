...
# The plots are created and stored in the working directory as .pdf files. Files with EMB contain embeded fonts for including in .pdf documents.
...
plot(c(0),c(0),
     type="l", 
     main=expression(paste("Contribution, ", G[r], " , as a function of perturbation ", delta)),
     #sub=paste("Species: ",NAME),
...
text(x = corners[1]-.18, y = mean(corners[3:4]), expression(G[r]), cex=1.75)
for(i in 1:num_nodes){
  par(new=TRUE)
  yplot <- PLOTGR[i,]
...
dev.off() ## END PLOT
embed_fonts(paste(NAME,"_GvsDelta_plots.pdf", sep=""), outfile=paste(NAME,"_GvsDelta_plots_EMB.pdf", sep=""))
...
pdf(paste(NAME,"_GRPvsDelta_plots.pdf", sep="")) ## START PLOT - save as .pdf file
symb <- 0
plot(c(0),c(0),
     type="l", 
     main=expression(paste("Pathway Criticality ", G[r], ", as a function of ", delta)),
     #sub=paste("Species: ",NAME),
...
corners = par("usr") #Gets the four corners of plot area (x1, x2, y1, y2)
par(xpd = TRUE) #Draw outside plot area
text(x = corners[1]-.12, y = mean(corners[3:4]), expression(G^r), cex=1.75)
...
dev.off() ## END PLOT
embed_fonts(paste(NAME,"_GRPvsDelta_plots.pdf", sep=""), outfile=paste(NAME,"_GRPvsDelta_plots_EMB.pdf", sep=""))
